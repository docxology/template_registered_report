...
Configuration lives in `manuscript/config.yaml`; preregistration fixtures live in `data/`; reusable plan-freezing, deviation, demonstration-analysis, and figure-provenance logic lives in `src/registered_report/` (`protocol.py`, `demo_study.py`, and `figures.py`). The manuscript binds every number to `output/data/demo_analysis.json` and every figure label to `output/figures/figure_registry.json`, both regenerated by `scripts/generate_figures.py`.
